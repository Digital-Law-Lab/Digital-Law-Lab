...
Only <DisplayAdminData data='github' /> should be able to merge changes into `main` and `next`.  Set up protection on the branch so these two boxes are checked:

![image](/img/docs/provision-protection.png)

:::info

Although the policy requires approval it does not restrict approvals only to <DisplayAdminData data='github' />.  However, only <DisplayAdminData data='github' /> should approve PRs into `main` and `next`.

:::

### Create Slack channel
On <DisplayServiceData data='slack' service_name='our Slack' /> create a new channel for this repository.
